...
export function computeMetrics(
  ...
  // Context switches: count of activity entries, minus the first entry per day
  ...
  for (let i = 0; i < sorted.length; i++) {
    ...
    for (const tag of a.tags) {
      ...
      // Count as a switch if the previous activity didn't involve this person
      if (i > 0) {
  ...
  const dailyBreakdown: DayMetrics[] = Array.from(dayMap.entries())
    ...
    .map(([date, dayActivities]) => {
      ...
      return {
        ...
        redMinutes: dRed,
        contextSwitches: Math.max(0, dayActivities.length - 1),
