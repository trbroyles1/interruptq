...
import { getDbDriver } from "@/db/config";
import { db } from "@/db/index";
import { run, returningFirst } from "@/db/helpers";
...
// ---------------------------------------------------------------------------
// Schema (conditional on DB driver)
// ---------------------------------------------------------------------------
...
// Token generation (same logic as src/lib/identity.ts)
...
export async function seedTestData(options: { weeks: number }): Promise<SeedResult> {
  const { weeks } = options;
  ...
  return {
    ...
    summary: {
      ...
      tickets: ticketPool.length + oncallTicketPool.length,
